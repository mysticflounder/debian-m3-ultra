...
for input in "$@"; do
    ...
    /usr/bin/jq -e -f "$HERE/scripts/validate-cssc-probe.jq" "$input" >/dev/null
done
/usr/bin/jq -n --arg host_path "$1" --arg guest_path "$2" --slurpfile host "$1" --slurpfile guest "$2" '
  ...
  [range(0;27) as $i | {op:$h[$i].op,input:$h[$i].input,
  ...
  {schema_version:1,host_capture:$host_path,guest_capture:$guest_path,
   ...
   limitation:"Only the sampled scalar immediate encodings and inputs; no full-feature claim"}'
