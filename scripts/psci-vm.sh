#!/bin/bash
# Exercise guest PSCI CPU hotplug across the M3 Ultra CPU-count
...
#
# First boot checks guest PSCI secondary CPU offline/online transitions on the
# existing Asahi builder kernel (not the proposed stock persistent kernel).
# A second boot verifies the disposable filesystem survives a clean relaunch.
...
#
#   ./scripts/psci-vm.sh
#   SMP_LIST="1 32" ./scripts/psci-vm.sh
#
...
OUT="$HERE/out"
HARNESS="$HERE/scripts/psci-vm.sh"
...
lsof_openers() {
    ...
    echo "psci probe: lsof failed for $path (status $status): $output" >&2
    return 1
}
...
cleanup() {
    ...
    if [ -n "$QEMU_PID_FILE" ] && { [ -e "$QEMU_PID_FILE" ] || [ -L "$QEMU_PID_FILE" ]; }; then
        if [ -f "$QEMU_PID_FILE" ] && [ ! -L "$QEMU_PID_FILE" ]; then
            ...
            echo "psci probe: refusing a replaced QEMU pid file" >&2
            cleanup_status=1
        fi
    fi
    QEMU_PID_FILE=""
    for overlay_path in "${OVERLAY_PATHS[@]}"; do
        if [ -L "$overlay_path" ]; then
            echo "psci probe: refusing a replaced symlink at overlay path: $overlay_path" >&2
            cleanup_status=1
        elif [ -f "$overlay_path" ]; then
            if ! openers="$(lsof_openers "$overlay_path")"; then
                cleanup_status=1
            elif [ -n "$openers" ]; then
                echo "psci probe: refusing to remove overlay still open by pid(s): $openers" >&2
                cleanup_status=1
            elif ! /bin/rm -f -- "$overlay_path"; then
                cleanup_status=1
            fi
        elif [ -e "$overlay_path" ]; then
            echo "psci probe: refusing unexpected object at overlay path: $overlay_path" >&2
            cleanup_status=1
        fi
        if [ -e "$overlay_path" ] || [ -L "$overlay_path" ]; then
            echo "psci probe: disposable overlay remains: $overlay_path" >&2
            cleanup_status=1
        fi
    done
    if [ "$PROTECTED_WINDOW_STARTED" = true ] && [ "$HASHES_READY" = true ] &&
       [ "$INPUTS_VERIFIED" = false ]; then
        if verify_protected_identities && hash_inputs_after; then
            ...
            echo "psci probe: a protected input identity or content changed during cleanup" >&2
            cleanup_status=1
        fi
    fi
...
RUN_DIR="$(/usr/bin/mktemp -d "$OUT/psci-matrix.XXXXXX")"
...
normalize_control_cleanup() {
    "$JQ" '
        .cycles |= map(if .attempted then . else .temporary_qemu_pid_file_removed = null end) |
        .safety.temporary_control_staging_removed =
            all(.cycles[] | select(.attempted); .temporary_qemu_pid_file_removed == true)
    ' "$1"
}

make_psci_script() {
    printf 'expected_cpus=%s\n' "$1"
    /bin/cat <<'PSCI_PAYLOAD'
cpu_root=/sys/devices/system/cpu
work_dir=''
restore_cpus() {
    local c failed=0
    for ((c=1; c<expected_cpus; c++)); do
        if [ -e "$cpu_root/cpu$c/online" ]; then
            printf '1\n' > "$cpu_root/cpu$c/online" || failed=1
        else
            failed=1
        fi
    done
    if [ -n "$work_dir" ]; then
        rm -f -- "$work_dir/work" "$work_dir/work.c" || failed=1
        rmdir -- "$work_dir" || failed=1
    fi
    [ "$(expanded_mask "$cpu_root/online")" = "$(expected_mask -1)" ] || failed=1
    [ "$(getconf _NPROCESSORS_ONLN)" -eq "$expected_cpus" ] || failed=1
    return "$failed"
}
expanded_mask() {
    local raw part first last c sep=''
    raw=$(cat "$1")
    local parts=()
...
expected_mask() {
    local excluded=$1 c sep=''
    for ((c=0; c<expected_cpus; c++)); do
        [ "$c" -ne "$excluded" ] || continue
        printf '%s%s' "$sep" "$c"; sep=,
    done
    printf '\n'
}
check_state() {
    local phase=$1 cpu=$2 excluded=$3 count=$4 mask present possible
    mask=$(expanded_mask "$cpu_root/online")
    present=$(expanded_mask "$cpu_root/present")
    possible=$(expanded_mask "$cpu_root/possible")
    [ "$mask" = "$(expected_mask "$excluded")" ]
    [ "$present" = "$(expected_mask -1)" ]
    [ "$possible" = "$present" ]
    [ "$(getconf _NPROCESSORS_ONLN)" -eq "$count" ]
    [ ! -e "$cpu_root/cpu0/online" ] || [ "$(cat "$cpu_root/cpu0/online")" = 1 ]
    printf '\nPSCI_STATE phase=%s cpu=%s online=%s mask=%s\n' "$phase" "$cpu" "$count" "$mask"
}
command -v taskset >/dev/null
taskset -pc 0 "$$" >/dev/null
[ "$(taskset -pc "$$" | awk '{print $NF}')" = 0 ]
check_state initial 0 -1 "$expected_cpus"
if [ "$expected_cpus" -gt 1 ]; then
    command -v gcc >/dev/null
    # /run is noexec on the builder guest; use its disposable root overlay.
    work_dir=$(mktemp -d /root/psci-work.XXXXXX)
    cat > "$work_dir/work.c" <<'PSCI_C'
#define _GNU_SOURCE
#include <sched.h>
#include <stdint.h>
#include <stdio.h>
...
int main(int argc, char **argv) {
    ...
    if (argc != 2) return 1;
    int cpu = atoi(argv[1]);
    if (cpu < 1 || cpu >= CPU_SETSIZE) return 2;
    if (sched_getaffinity(0, sizeof allowed, &allowed) ||
        CPU_COUNT(&allowed) != 1 || !CPU_ISSET(cpu, &allowed)) return 3;
    volatile uint64_t sum = 0;
    for (uint64_t i=1; i<=10000; i++) {
        if (sched_getcpu() != cpu) return 4;
        sum += i;
    }
    if (sum != 50005000) return 5;
    printf("\nPSCI_WORK cpu=%d sum=%llu observed=%d\n", cpu,
           (unsigned long long)sum, sched_getcpu());
    return 0;
}
PSCI_C
    gcc -O2 -Wall -Wextra -Werror -o "$work_dir/work" "$work_dir/work.c"
fi
for ((cpu=1; cpu<expected_cpus; cpu++)); do
    [ -w "$cpu_root/cpu$cpu/online" ]
    printf '0\n' > "$cpu_root/cpu$cpu/online"
    [ "$(cat "$cpu_root/cpu$cpu/online")" = 0 ]
    check_state offline "$cpu" "$cpu" "$((expected_cpus-1))"
    printf '1\n' > "$cpu_root/cpu$cpu/online"
    [ "$(cat "$cpu_root/cpu$cpu/online")" = 1 ]
    check_state online "$cpu" -1 "$expected_cpus"
    taskset -c "$cpu" "$work_dir/work" "$cpu"
done
restore_cpus
work_dir=''
check_state final 0 -1 "$expected_cpus"
if [ "$expected_cpus" -eq 1 ]; then
    printf '\nPSCI_TRANSITIONS status=not-applicable count=0\n'
else
    printf '\nPSCI_TRANSITIONS status=passed count=%s\n' "$((expected_cpus-1))"
fi
PSCI_PAYLOAD
}

validate_psci() {
    local smp=$1 console=$2 dir=$3
    "$AWK" -v n="$smp" '
        function mask(exclude, i,s) {
            s=""; for(i=0;i<n;i++) if(i!=exclude) s=s (s=="" ? "" : ",") i
            return s
        }
        BEGIN {
            print "PSCI_STATE phase=initial cpu=0 online=" n " mask=" mask(-1)
            for(c=1;c<n;c++) {
                print "PSCI_STATE phase=offline cpu=" c " online=" n-1 " mask=" mask(c)
                print "PSCI_STATE phase=online cpu=" c " online=" n " mask=" mask(-1)
                print "PSCI_WORK cpu=" c " sum=50005000 observed=" c
            }
            print "PSCI_STATE phase=final cpu=0 online=" n " mask=" mask(-1)
            print "PSCI_TRANSITIONS status=" (n==1 ? "not-applicable" : "passed") " count=" n-1
        }' > "$dir/expected-transitions.txt"
    "$AWK" '/^PSCI_(STATE|WORK|TRANSITIONS) /' "$console" > "$dir/observed-transitions.txt"
    "$JQ" -Rn '[inputs | split(" ") | {event:.[0], fields:(.[1:] | map(split("=") | {key:.[0],value:.[1]}) | from_entries)}]' \
        < "$dir/observed-transitions.txt" > "$dir/transitions.json"
    /usr/bin/cmp -s "$dir/expected-transitions.txt" "$dir/observed-transitions.txt"
}
...
guest_shutdown() {
    rc=\$?
    trap - EXIT
    if declare -F restore_cpus >/dev/null; then
        restore_cpus || { echo 'PSCI_RESTORE_FAILED'; rc=1; }
    fi
    if [ "\$rc" -ne 0 ]; then
        echo "PSCI_GUEST_ERROR cycle=1 rc=\$rc"
    fi
    sync
    echo "PSCI_CLEAN_SHUTDOWN_REQUESTED cycle=1"
    systemctl poweroff || true
    sleep 300
}
trap guest_shutdown EXIT
printf '\\nPSCI_CYCLE_BEGIN cycle=1\\n'
$(make_psci_script "$smp")
online=\$(getconf _NPROCESSORS_ONLN)
echo "PSCI_CPU cycle=1 configured=$smp online=\$online"
install -d -m 700 /var/lib/m3-hvf-psci
printf '%s\\n' '$token' > /var/lib/m3-hvf-psci/sentinel.new
chmod 600 /var/lib/m3-hvf-psci/sentinel.new
mv /var/lib/m3-hvf-psci/sentinel.new /var/lib/m3-hvf-psci/sentinel
sync
IFS= read -r observed < /var/lib/m3-hvf-psci/sentinel
[ "\$observed" = '$token' ]
echo "PSCI_SENTINEL_WRITE_OK cycle=1"
echo "PSCI_CYCLE_COMPLETE cycle=1"
trap - EXIT
sync
echo "PSCI_CLEAN_SHUTDOWN_REQUESTED cycle=1"
systemctl poweroff
...
guest_shutdown() {
    ...
    trap - EXIT
    if [ "\$rc" -ne 0 ]; then
        echo "PSCI_GUEST_ERROR cycle=2 rc=\$rc"
    fi
    sync
    echo "PSCI_CLEAN_SHUTDOWN_REQUESTED cycle=2"
    systemctl poweroff || true
    sleep 300
}
trap guest_shutdown EXIT
printf '\\nPSCI_CYCLE_BEGIN cycle=2\\n'
online=\$(getconf _NPROCESSORS_ONLN)
echo "PSCI_CPU cycle=2 configured=$smp online=\$online"
[ -f /var/lib/m3-hvf-psci/sentinel ]
IFS= read -r observed < /var/lib/m3-hvf-psci/sentinel
[ "\$observed" = '$token' ]
echo "PSCI_SENTINEL_PERSISTED_OK cycle=2"
echo "PSCI_CYCLE_COMPLETE cycle=2"
trap - EXIT
sync
echo "PSCI_CLEAN_SHUTDOWN_REQUESTED cycle=2"
systemctl poweroff
...
run_cycle() {
    ...
    (
        ...
        printf 'stty -echo\n' >&9
        printf "/bin/bash <<'PSCI_GUEST_EOF'\n%s\nPSCI_GUEST_EOF\n" \
            "$guest_script" >&9
    ) &
    FEED_PID=$!
    ...
    /usr/bin/env -i \
        ...
        psci-qemu-launch "$QEMU_PID_FILE" "$QEMU" "${ARGS[@]}" \
        <&9 > "$log" 2>&1 &
    QPID=$!
    ...
    [ "$(marker_count "PSCI_CYCLE_BEGIN cycle=$cycle" "$console")" -eq 1 ] &&
        CYCLE_BEGIN=true
    ...
    if [ "$cycle" -eq 1 ]; then
        [ "$(marker_count "PSCI_SENTINEL_WRITE_OK cycle=1" "$console")" -eq 1 ] &&
            CYCLE_SENTINEL=true
    else
        [ "$(marker_count "PSCI_SENTINEL_PERSISTED_OK cycle=2" "$console")" -eq 1 ] &&
            CYCLE_SENTINEL=true
    fi
    [ "$(marker_count "PSCI_CYCLE_COMPLETE cycle=$cycle" "$console")" -eq 1 ] &&
        CYCLE_COMPLETE=true
    [ "$(marker_count "PSCI_CLEAN_SHUTDOWN_REQUESTED cycle=$cycle" "$console")" -eq 1 ] &&
        CYCLE_CLEAN_SHUTDOWN=true
    [ "$("$AWK" -v cycle="$cycle" \
        '$0 ~ ("^PSCI_GUEST_ERROR cycle=" cycle " rc=[0-9]+$") { n++ } END { print n + 0 }' \
...
run_count() {
    ...
    token="m3-hvf-psci-smp${smp}-$(/bin/date +%s)-$RANDOM$RANDOM"
    token_sha256="$(sha256_text "$token")"
    echo "booting psci cycle 1: ${smp} vCPUs, ${MEM} RAM -> $COUNT_DIR"
    ...
    if ! validate_psci "$smp" "$CYCLE1_CONSOLE" "$COUNT_DIR"; then
        c1_valid=false
    fi

    if [ "$c1_valid" = true ]; then
        c2_attempted=true
        echo "booting psci cycle 2 (relaunch): ${smp} vCPUs"
    ...
    echo "verifying protected inputs after ${smp}-vCPU psci test"
    if verify_protected_identities; then
    ...
    if [ "$c1_valid" = true ] && [ "$c2_valid" = true ] &&
       ...
       [ "$inputs_unchanged" = true ] && [ "$identities_stable" = true ]; then
        ...
        failure_reason="a protected input filesystem identity changed during the psci test"
    elif [ "$c1_valid" != true ]; then
        failure_reason="cycle 1 did not satisfy the psci marker and shutdown contract"
        ...
        failure_reason="a protected input changed during the psci test"
    fi
    ...
    "$JQ" -n \
        ...
        --arg memory "$MEM" \
        --arg mode "relaunch" \
        --arg sentinel_path "/var/lib/m3-hvf-psci/sentinel" \
        ...
        --argjson pass "$result" '
        {
            ...
            safety: {
                ...
                psci_is_relaunch_not_reboot: true,
    ...
    normalize_control_cleanup "$evidence_tmp" > "$COUNT_DIR/.cleanup-evidence.json"
    /bin/mv "$COUNT_DIR/.cleanup-evidence.json" "$evidence_tmp"
    if ! "$JQ" -e --argjson expected_argv "$qemu_argv_json" '
        .schema_version == 1 and .psci_mode == "relaunch" and
        ...
        .safety.psci_is_relaunch_not_reboot == true and
    ...
    ' "$evidence_tmp" >/dev/null; then
        fail "generated psci evidence failed structural validation"
    fi
    "$JQ" --slurpfile transitions "$COUNT_DIR/transitions.json" \
        --argjson smp "$smp" --argjson valid "$c1_valid" '
        .guest_baseline = "existing-asahi-builder-kernel" |
        .stock_persistent_kernel_tested = false |
        .psci = {controller_cpu:0, evidence:$transitions[0],
            expected_secondary_transitions:($smp-1),
            transitions_passed:($valid and $smp>1),
            status:(if $smp==1 then "not-applicable" elif $valid then "passed" else "failed" end)}
    ' "$evidence_tmp" > "$COUNT_DIR/.psci-evidence.json"
    /bin/mv "$COUNT_DIR/.psci-evidence.json" "$evidence_tmp"
    ...
    echo "psci evidence: $evidence"
}

echo "hashing protected inputs before psci matrix"
hash_inputs_before
...
' "$MANIFEST_TMP" >/dev/null; then
    fail "aggregate psci manifest failed structural validation"
fi
