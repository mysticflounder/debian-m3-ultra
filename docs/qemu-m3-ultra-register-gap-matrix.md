...
| PFR2, ISAR2, MMFR3, MMFR4 | unavailable through selected named API (named-API gap only) | SDK 26.5 supplies no named configuration-query enums for these. Effective guest exposure is unmeasured, not proven absent or wrong; do not invent values from a chip name. |
