...
| ISAR2.RPRES specifically | advertisement mismatch; sampled behavior matches host | The [read-only host-feature audit](qemu-m3-ultra-public-hvf-controls.md) measured macOS `FEAT_RPRES=1` against guest RPRES `[7:4]=0`. The subsequent [instruction comparison](qemu-m3-ultra-rpres-behavior.md) matched all 28 controlled host/guest results. No raw host ISAR2 capture or complete behavioral equivalence is claimed; no feature override. |
